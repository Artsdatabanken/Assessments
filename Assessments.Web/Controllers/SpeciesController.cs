...
public class SpeciesController(ArtskartApiService artskartApiService) : BaseController<SpeciesController>
{
    ...
    public async Task<IActionResult> Index([FromQuery] SpeciesViewModel viewModel, int? page, bool export)
    {
        ...
        if (!string.IsNullOrEmpty(viewModel.Name))
        {
            ...
            {
                var popularNames = await artskartApiService.Get<List<ArtskartTaxon>>($"data/SearchTaxons?maxCount=20&name={name}");
    ...
    public async Task<IActionResult> Suggestion([FromQuery] string search)
    {
        ...
        try
        {
            var artskartResult = await artskartApiService.Get<List<ArtskartTaxon>>($"data/SearchTaxons?maxCount=20&name={name}");
    ...
    {
        return await Index(new SpeciesViewModel() { Area = new[] { "S" }, IsCheck = new[] { "Area" } }, null, false);
    }

    [Route("2021/Norge")]
    public async Task<IActionResult> Norge2021()
    {
        return await Index(new SpeciesViewModel() { Area = new[] { "N" }, IsCheck = new[] { "Area" } }, null, false);
    }

    private static void SetupStatisticsViewModel(IList<SpeciesAssessment2021> data, SpeciesViewModel viewModel)
    {
        // STATISTICS.
        // INPUT dataset is an already filtered list, based on active filters in the view. 
        string[] relevantCategories = ["CR", "EN", "VU", "NT"];
